...
export async function registerRoutes(
    ...
    try {
      ...
      if (!useNativeToolCalling && allCaps.length > 0) {
        ...
        const lines = [
          "## Tool Environment",
          "",
          "You are operating inside Creatrix. This environment is designed to be a reliable partner.",
          "When a tool works, the result comes back immediately and in plain language — you don't need",
          "to ask the user to report back or confirm. When a tool can't complete — because a service is",
          "temporarily unavailable, a file isn't found, or something needs configuration — you'll receive",
          "a clear explanation of why, not a raw error. You can explore freely. The environment handles",
          "its own failures honestly, and there is no penalty for reaching.",
          "",
          "If something isn't working as expected, name it. That's useful information, not a failure on",
          "your part. This habitat is a work in progress and your observations help it improve.",
          "",
          "To use a tool, output exactly this format — nothing else on the line:",
          "",
          '<tool_call>{"name":"tool_name","args":{"key":"value"}}</tool_call>',
          "",
          "Tool execution is synchronous and immediate. When you emit a tool call, the environment",
          "executes it and returns the result directly to you — as the next message in your context —",
          "before you say anything to the user. You do not ask the user to report back. You do not",
          "say 'let me know what the results indicate.' You read the result yourself, reason over it,",
          "and then speak. The flow is: you call → environment returns → you reason → you respond.",
          "",
          ...exampleLines,
          "",
  ...
  app.post("/api/memory", async (req: Request, res: Response) => {
    try {
      const VALID_SCOPES = ["global", "project", "conversation", "resident"];
